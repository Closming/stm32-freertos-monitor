...
/** 单次 I²C 事务拿锁超时。
 *
 * ★ 2026-09-26 由 100 调到 300：100 这个值当初没有依据，也没考虑过
 *   ssd1306_flush() 的分片持锁时长。真机实测（SHT30 接上后）TaskSensor
 *   每次读都拿不到锁 → ALARM_SENSOR_ERR 常亮、OLED 上温湿度恒为 --.--。
 *
 *   为什么 100 不够：TaskSensor 是最高优先级，等锁时持锁的 TaskDisplay
 *   会继承它的优先级，**最多等一个分片**就能拿到 —— 所以每次等穿 100ms
 *   只能说明"单个分片 > 100ms"（256 字节数据）。
 *
 *   ⚠️ 这是一个**诊断性的放宽，不是根治**。它把 timeout 这个遮蔽去掉，
 *   让通信层（nack / crc）能露出来。真正的瓶颈是软件 I²C 的位速率
 *   （见上面的 I2C_DELAY_US 与 scl_release_and_wait 的取舍）。
 *   TaskSensor 周期 1000ms，等 300ms 在实时性上可以接受。
 *
 *   ⚠️ 副作用：等锁期间 TaskDisplay 被提升到最高优先级，会挤占
 *   TaskAcquire（周期 50ms）等中等优先级任务最多 300ms。
 *   若之后看到 WDT STALLED 之类的日志，先怀疑这里。 */
#define I2C_TIMEOUT_MS      300u
 ...
 * ——ADC 输入超过 VDDA 会打坏引脚。
 *
 * ★ 光敏接的是现成**模块**的 `AO` 输出（板载分压），**不是**自制分压网络。
 *   ⚠️ 2026-09-26 更正：此处原写「光敏用 10kΩ 上拉分压」，那只是选型时的设想，
 *   实际没这么搭。`AO` 脚本质就是"光敏电阻 + 固定电阻"的分压点，接 ADC 读到的
 *   仍是分压值，**代码侧不受影响**；但描述硬件时**不要说自己搭了分压电路**。
 * ========================================================================== */
