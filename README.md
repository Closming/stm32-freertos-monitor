...
| **整个 Keil 工程链接通过** | ✅ **`0 Error(s), 0 Warning(s)`**，`Code=26240 + RO=1648 + RW=236` ≈ **27.5 KB Flash**，`ZI=16564` ≈ **16.8 KB RAM (82%)** |
...
| 串口 / USART1+DMA / 自定义协议 / 上位机 | ⏳ **未验证** —— CH340 已到货，但尚未接线，全程零串口记录 |
| ADC 双通道 / 按键消音 / 蜂鸣器 / IWDG | ⏳ **未验证** —— 真机调试尚未推进到这些外设 |
...
- **真机验证只覆盖了一部分**。已确认可用的是：调度器与任务调度、ST-Link 烧录链路、
  软件 I²C 电气层与命令通道、**OLED 显示**。**尚未在真机上验证的是**：SHT30 读数
  （器件未到货）、串口与上位机链路（CH340 未接线）、ADC 双通道、按键消音、蜂鸣器、IWDG。
